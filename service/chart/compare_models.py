...
def plot_comparison(sentences, model1_results, model2_results, save_path=None):
    model1_scores = [
        res["score"] + 1 for res in model1_results
    ]  # 模型 1 的情感分數
    model2_scores = [
        res["score"] + 1 for res in model2_results
    ]  # 模型 2 的情感分數
    ...
    plt.xticks(
        range(len(sentences)),
        [f"Text {i+1}" for i in range(len(sentences))],
        rotation=45,
    )
...
def plot_distribution(model1_results, model2_results, save_path=None):
    ...
    sns.kdeplot(
        model1_scores, shade=True, color="blue", label="Model 1", alpha=0.7
    )
    sns.kdeplot(
        model2_scores, shade=True, color="red", label="Model 2", alpha=0.7
    )
