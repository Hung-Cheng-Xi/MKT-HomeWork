...
avg_ratings = {food: sum(ratings)/len(ratings) 
             ...
             if food_counts[food] >= 15}
# 取平均分數前5高的食物
top_5 = sorted(avg_ratings.items(), key=lambda x: x[1], reverse=True)[:5]
...
avg_score = sum(item['rank'] for item in data) / total_count
...
plt.axhline(y=avg_score, color='r', linestyle='--', label=f'平均分數: {avg_score:.2f}')

plt.title(f'前5名最高評分食物', fontsize=12)
plt.xlabel('食物', fontsize=10)
plt.ylabel('平均評分', fontsize=10)
plt.xticks(rotation=45)
plt.legend()
plt.savefig('data/custom_data/score_top5.png')
plt.tight_layout()
